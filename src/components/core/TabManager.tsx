  ...
  return (
    <ShadTabs value={activeTabId || ""} onValueChange={setActiveTabId} className="flex flex-col flex-grow h-full">
      <TabsList className="bg-muted print:hidden rounded-none border-b border-border justify-start px-2">
        {tabs.map((tab) => (
          <TabsTrigger
            key={tab.id}
            value={tab.id}
            className="relative group data-[state=active]:bg-background data-[state=active]:shadow-sm data-[state=active]:text-primary rounded-t-md rounded-b-none border-t border-x border-transparent data-[state=active]:border-border data-[state=active]:border-b-background"
          >
            ...
            {tab.closable !== false && (
              <Button
                variant="ghost"
                size="icon"
                className="ml-2 h-5 w-5 opacity-50 group-hover:opacity-100 data-[state=active]:opacity-100"
                onClick={(e) => {
      ...
      {tabs.map((tab) => (
       ...
       {/* Fallback content if no tab is active or content not found, though TabContentRenderer handles its own fallback */}
